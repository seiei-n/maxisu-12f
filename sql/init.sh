...
ISUCON_DB_HOST_1=${ISUCON_DB_HOST_1:-127.0.0.1}
ISUCON_DB_HOST_2=${ISUCON_DB_HOST_2:-127.0.0.1}
...
mysql -u"$ISUCON_DB_USER" \
		...
		--port "$ISUCON_DB_PORT" \
		"$ISUCON_DB_NAME" < 3_schema_user_series_db1.sql

mysql -u"$ISUCON_DB_USER" \
		-p"$ISUCON_DB_PASSWORD" \
		--host "$ISUCON_DB_HOST_2" \
		--port "$ISUCON_DB_PORT" \
		"$ISUCON_DB_NAME" < 4_schema_exclide_user_presents_db2.sql
...
DIR=`mysql -u"$ISUCON_DB_USER" -p"$ISUCON_DB_PASSWORD" -h "$ISUCON_DB_HOST_2" -Ns -e "show variables like 'secure_file_priv'" | cut -f2`
